...
#  generated by pics.
#  TODO: we need to set dependency on cmakelist
import flatbuffers
...
class TorchQParamExporter:
    def __save_np(self, data):
        ...
        if data.shape == ():
            data = np.array([data])
        #  python interpreter(64bit) may be uses float64 for default
    ...
    def __init__(self, quantized_model, json_path):
        ...
        self.__input_set = None
        self.__output_set = None
    ...
    def set_mapping(self, original_model, sample_input):
        ...
        circle = self.__convert_original_model(sample_input, param_name_set)
    ...
    def __convert_original_model(self, sample_input, param_name_set):
        original_model = self.__original_model
        dir_path = os.path.join(self.__dir_path, "tmp")
        if not os.path.exists(dir_path):
            os.makedirs(dir_path, exist_ok=True)

        operator_candidate = set()

        for name, mod in original_model.named_modules():
            if name == '':  # it's just model itself
                continue
            if isinstance(mod, torch.quantization.QuantStub):
                self.__input_set.add(name)
            elif isinstance(mod, torch.quantization.DeQuantStub):
                self.__output_set.add(name)
            else:  # After mapping, when
                operator_candidate.add(name)

        # operator_candidate is not mapped yet.
        # we need to map by directed graph
        operator_candidate = operator_candidate - param_name_set
        # TODO: do it!
        ...
        tf_dict = MessageToDict(graph_def)
        ...
        for name, layer in tree.items():
            ...
            if "weight" in layer:
                ...
                if tensor.is_quantized:
                    dtype = self.__qdtype_mapping[tensor.dtype]['str']
            ...
            if "scale" in layer and "zero_point" in layer:
                ...
                if "bias" in layer:
                    quantized_bias = quantize_tensor(layer['bias'], scale, zero_point)
                    data[b_name] = {
                        'scale': s_np,
                        'zerop': z_np,
                        'dtype': dtype,
